...
/*   Updated: 2022/08/25 18:24:00 by gusousa          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_append(char *line, char *s2, int len_stash)
{
	...
	if (!new)
		return (0);
	ft_strlcpy_g(new, line, line_len + 1);
	ft_strlcpy_g(&new[line_len], s2, len_stash + 1);
...
char	*divide_et_vince(int fd, char *line, char *stash)
{
	...
	if (stash[0] != '\0')
	{
		len_stash = ft_strlen_g(stash, &end_flag);
		line = ft_strdup_g(stash, len_stash);
	}
	while (end_flag == 0)
	{
